  ...
  */
  ...
  public postReview(id, review) {
    return this.http.post(
      this.baseUrl + "/product/updateReviews/" + id,
      review
    );
  }
  ...
  public getProductByCategory(idCategory): Observable<any[]> {
    return this.http.get<any[]>(
      this.baseUrl + "/product/findProductByCategory/" + idCategory
    );
  }
  public verifEmailReview(productId, email): Observable<any[]> {
    return this.http.get<any[]>(
      this.baseUrl +
        "/product/verifExistEmailOnReviews/" +
        productId +
        "/" +
        email
    );
  ...
  */
  ...
  public sortProducts(products: Product[], payload: string): any {
    if (payload === "ascending") {
      ...
      return products.sort((a, b) => {
        if (
          a?.price - (a?.price * a?.remise) / 100 <
          b?.price - (b?.price * b?.remise) / 100
        ) {
          return -1;
        } else if (
          a?.price - (a?.price * a?.remise) / 100 >
