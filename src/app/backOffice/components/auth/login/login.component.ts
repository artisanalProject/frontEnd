...
export class LoginComponent implements OnInit {
  ...
  login() {
    if (this.loginForm.invalid) {
      ...
      this.artisanService.login(this.loginForm.value).subscribe(
        (res) => {
        ...
        () => {
          if (this.responseLogin == "verify email or password") {
            ...
            this._snackBar.open(
              "Votre compte n'est pas activé. Vous recevrez un mail d'activation le plutôt possible",
              "ok",
              {
                duration: 10000,
                verticalPosition: "top",
            ...
            localStorage.setItem(
              "connectedUser",
              JSON.stringify(this.responseLogin)
            );
  ...
  register() {
    if (this.registerForm.invalid) {
      ...
      this.artisanService.register(this.registerForm.value).subscribe(
        (res) => {
          this.responseRegister = JSON.parse(JSON.stringify(res));
        },
        (err) => {
          console.log(err);
        },
        () => {
          if (this.responseRegister == "account already exist") {
            ...
            this._snackBar.open(
              "Vous êtes inscri avec succès, vous recevrez un email de confirmation le plutôt possible",
              "ok",
              {
                duration: 10000,
                verticalPosition: "top",
  ...
  forgotPwd() {
    if (this.loginForm.controls.email.value) {
      this.adminService
        .forgotPwd(this.loginForm.controls.email.value)
        .subscribe(
          (res) => {
            this.responseForgotPwd = JSON.parse(JSON.stringify(res));
          },
          (err) => {},
          () => {
            if (this.responseForgotPwd == "Email sent!") {
              this._snackBar.open(
                "Un email de récupération de mot de passe est envoyé",
                "ok",
                {
                  duration: 5000,
                  verticalPosition: "top",
